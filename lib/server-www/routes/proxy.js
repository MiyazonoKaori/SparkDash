...
module.exports = function(app){
	...
	app.get('/:_oid/_devices', function(req, res){
		...
		var request = http.request({
		    	...
		    	method: 'GET',
					headers:{'Authorization':'test'},
		    	path: '/api/devices?app_oid='+req.params._oid
				}, function(__res){
					var str = '';
				  __res.on('data', function (chunk) {
				    str += chunk;
	...
	app.get('/:_oid/:_clientID/status', function(req, res){
		// GET Proxy request to API server
		var request = http.request({
		    	port: 3001,
		    	hostname: '127.0.0.1',
		    	method: 'GET',
					headers:{'Authorization':'test'},
		    	path: '/api/'+req.params._clientID+'/status?app_oid='+req.params._oid
				}, function(__res){
					var str = '';
				  __res.on('data', function (chunk) {
				    str += chunk;
