...
module.exports = function(app, App){
	...
	app.get('/:_id/devices', function(req, res){
				...
				App.db.mongo.apps.findOne({_id:App.db.mongo.ObjectId(req.params._id)},function(_err,_app){
					...
					if (_app) {
						...
						var request = http.request({
						    	...
						    	path: '/api/devices?app_oid='+req.params._id+'&filter=clientID'
								}, function(__res){
								  ...
								  __res.on('end', function () { 
										...
										if (req.headers['x-requested-with'] == 'XMLHttpRequest') {
											res.json({'status':devices.status, 'message':devices.message}); 
										} else {
											res.render('devices', {'app':_app, 'domain':req.subdomain, 'devices':devices.message, 'ws':{'channel':req.subdomain.id+'_'+_app.appPackage}});
										}
										devices = null;
								  });
								}
