...
SPMongo_connect(function(err, db) {
  if (err) { return console.log('errrrrrrrr!'); }
    ...
    if(dberr) {
			...
			// 			}
			console.log('SPMongo is NOT connected to '+config.databases.mongo);
			errObj = {errconn:{message:'Database is offline for this domain.'}};
		} else {
			console.log('SPMongo is connected to '+config.databases.mongo);
			errObj = {connected:{message:'This domain is not registered.'}};
		}
	});
...
module.exports = function(req, res, next) {
	...
	SPMongo.domains.findOne({id:subdomain},function(err,domain){
		if (domain) {
			...
			res.render('register_domain',errObj);
		}
	});
