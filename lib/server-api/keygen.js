...
keygen.generateKeys = function(args,_cb) {
		console.log(args);
		// Create key for package
		var crypted = keygen.createKey(args.seed,args.pkg+'|'+args.user_id+'|'+args.expires+'|'+args.level);
...
keygen.generateAndUpdate = function(args,_cb){ 
	...
	// required
	// seed, app_id, user, level, timestamp(expiration)
					...
					if (app) {
						...
						if (app.key) {
							...
							db.apps.findAndModify({
							    query: { pkg: args.pkg, user_id:args.user_id },
