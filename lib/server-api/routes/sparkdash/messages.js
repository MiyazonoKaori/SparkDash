							...
							req.Redis.SETEX(key, 30, JSON.stringify(req.body), function(e,o){
								...
								if (req.body.hasOwnProperty('devices')) {
									...
									for(var x in req.body.devices) {
										pusher.trigger(WS_MainChannel, evt, req.body.message,req.body.devices[x],function(){
											if(x == req.body.devices.length) {
									...
									// Explicity use the socketID if it's set in the body
									var socketID = (req.body.message.socketID) ? req.body.message.socketID : false;
									pusher.trigger(WS_MainChannel, evt, req.body.message,socketID,function(){
										res.json({'status':200,'message':{'event':evt,'payload':req.body.data}});
									});
