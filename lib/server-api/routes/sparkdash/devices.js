								...
								if (devices) {
									devices.map(function(d){
										if(d.appPackage == app.appPackage) {
											...
											if (req.query.device_state) {
												switch(req.query.device_state) {
													case 'all':
														filtered.push(d);
														break;
													case 'any':
														...
														break;
													default:
														if (d.device_state == req.query.device_state) {
															filtered.push(d);
														}
												}
												...
												// Default
												// Show only device_states higher than 2 (those at least logged in)
												if (d.device_state > 2) {
													filtered.push(d);
												}
