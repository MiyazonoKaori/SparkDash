...
var getExceptions = function(obj, res, _cb){
	var request = http.request({
	    	...
	    	path: '/tables/devices/query'
			  ...
			  __res.on('end', function () { 
					obj.exception_count = JSON.parse(str).count||'?';
					_cb(obj, res);
			  });
			}
	);
	request.write("WHEN appPackage == '"+obj.appPackage+"' THEN SELECT count() END");
	request.end();
};

var getEvents = function(obj, res, _cb){
	var request = http.request({
	    	port: 8585,
	    	hostname: '127.0.0.1',
	    	method: 'POST',
						...
						App.db.mongo.apps.findOne(query,function(err,app) {
							if (app) {
											...
											if (req.query.lifecycle_state) {
												...
												switch(req.query.filter) {
													case 'sum':
														...
														if (d.userID.length > 1 || !d.userID.length == "false") {
															userCount++;
														}
														break;
													case 'clientID':
								...
								if (Filtered == 'sum') {
									...
									getEvents(obj, res, function(obj, res){
										getExceptions(obj, res, function(obj, res){
