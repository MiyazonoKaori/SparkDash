...
var getCustomEvent = function(req,res,_keyTimestamp,cb) {
	...
	var pushObj = {
		evt:(req.body.eventName) ? 'custom_'+req.body.eventName+'@beacon' : 'custom_unknown@main',
		payload:{}
	};
...
var getInternalEvent = function(req,res,_keyTimestamp,cb) {
	...
	var evt = 'internal_unknown@beacon';
	var payload = {};
	...
	switch( req.body.eventName.toUpperCase() ) {
		...
		// CLIENT LIFECYCLE
		case 'LOGON':
			evt = 'internal_logon@beacon';
			payload = {};
			cb(res,WS_MainChannel, {event:evt,payload:payload}, socketID);
			break;

		case 'LOGOUT':
			evt = 'internal_logout@beacon';
			payload = {};
			cb(res,WS_MainChannel, {event:evt,payload:payload}, socketID);
			break;

		case 'START APP':
			evt = 'internal_startapp@beacon';
			payload = {};
			
			db.apps.findOne({pkg:req.body.appPackage},function(err,app){
				payload.current_version = (app) ? app.current_version : false;
				cb(res,WS_MainChannel, {event:evt,payload:payload}, socketID);
			});
		...
		case 'LOCATION':
			...
			case 'REMOVE':
				...
				evt = 'internal_removeclient@beacon';
			 	payload = {
					clientID: req.body.clientID,
					userID:req.body.userID,
