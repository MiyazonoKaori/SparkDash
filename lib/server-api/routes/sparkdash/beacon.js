				...
				Redis.HEXISTS(req.body.clientID, "clientID",function(_err,_hexists){
					...
					Redis.HMSET(req.body.clientID, req.body, function(e,o) {
						...
						if (_hexists) {
							...
							pusher.trigger(req.subdomain.id+'_'+req.body.appPackage+'_'+req.body.appID, 'update_client_geo@beacon', {
							...
							// Broadcast websocket
							// channels are based on domain -> pkg -> appid -> client (optional)
							pusher.trigger(req.subdomain.id+'_'+req.body.appPackage+'_'+req.body.appID, 'new_client@beacon', {
								clientID: req.body.clientID,
								enabled:req.body.enabled,
								latitude: req.body.latitude, 
								longitude: req.body.longitude,
