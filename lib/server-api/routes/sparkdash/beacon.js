			...
			db.apps.findOne({appPackage:req.deviceMeta.appPackage},function(err,app){
				...
				if (req.deviceMeta.appBuild != app.current_build) {
					...
					payload = {
						...
						clientID				: [{"clientID":req.deviceMeta.clientID}],
						userID					: [{"userID":req.deviceMeta.userID}],
			...
			req.Redis.select(2, function() {
				...
				req.Redis.HGET(req.body.clientID, "data",function(_err,_data){
					...
					req.deviceMeta = {
						...
						data						: (_data) ? JSON.parse(_data) : {status:{}}
					}
					
					req.deviceMeta.latitude		= (req.body.hasOwnProperty('latitude')) ? req.body.latitude : _data.latitude;
					req.deviceMeta.longitude	= (req.body.hasOwnProperty('longitude')) ? req.body.longitude : _data.longitude;
					req.deviceMeta.geohash		= false;
					req.deviceMeta.status			= (_data) ? _data.status : {};
					...
					if (req.body.hasOwnProperty('latitude')) {
						req.deviceMeta.data.latitude = req.body.latitude;
						delete req.body.latitude;
					}
					if (req.body.hasOwnProperty('longitude')) {
						req.deviceMeta.data.longitude = req.body.longitude;
