...
var getLifecycleState = function(meta) {
	var States = {
			"STOP APP"		: 0,
			"STOP"				: 0,
			"START APP"		: 1,
	...
	if(meta.eventType == 'EXCEPTION') {
		meta.eventName = 'EXCEPTION';
	}
	
	if (States.hasOwnProperty(meta.eventName)) {
		return States[meta.eventName];
	} else {
		return -1;
	}
};
...
var getTaskEvent = function(App,req,res,meta,cb){
	...
	var evt = (meta.eventName) ? 'task_'+meta.eventName + '@beacon' : 'task_unknown@beacon';
	var payload = {
		action:"task",
		appPackage			: req.deviceMeta.appPackage,
		appID						: req.deviceMeta.appID,
	...
	cb(App,res,WS_MainChannel, {event:evt,payload:payload}, socketID); 
};
var getExceptionEvent = function(App,req,res,meta,cb){
	...
	cb(App,res,WS_MainChannel, {event:false,payload:payload}, socketID);
};
var getInternalEvent = function(App,req,res,meta,cb) {
	...
	switch( meta.eventName ) {
		...
		case 'LOGOUT':
			...
			req.Redis.HGET(req.deviceMeta.clientID, "data", function(_err,_data){
				if (_err) { res.send({status:503,message:e.message}); return; }
					...
					// remove status ID 
					req.Redis.HMSET(req.deviceMeta.clientID, {data:JSON.stringify(d)}, function(e,o) {
			...
			req.Redis.select(2, function() {
				...
				req.Redis.HGET(req.body.clientID, "data",function(_err,_data){
					...
					var eventMeta = {
						...
						eventName:(req.body.eventName) ? req.body.eventName.toUpperCase() : '',
					...
					req.deviceMeta = {
						...
						lifecycle_state	: getLifecycleState(eventMeta),
