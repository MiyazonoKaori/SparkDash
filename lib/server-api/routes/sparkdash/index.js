...
module.exports = function(app,App){
	...
	app.all('/api', function(req, res) {
		...
		App.db.mongo.domains.findOne({id:subdomain},function(err,domain){
			if (domain) {
				res.json(200,{'status':200,'message':'Welcome to '+domain.title+' API.'});
			} else {
				res.json(200,{'status':200,'message': 'This domain is not registered.'});
			}
		});
	...
	app.get('/api/sparkdash', function(req, res) {
		...
		res.json(200,{'status':200,'message':'SparkDash API'});
	});

	// Catcha all
	app.all('/api/sparkdash/:do', function(req, res){
		res.json(200,{'status':200,'message':'invalid verb. Use POST.'});
	});
